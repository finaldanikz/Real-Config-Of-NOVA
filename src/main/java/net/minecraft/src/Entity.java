...
public abstract class Entity {
	...
	protected void setRotation(float par1, float par2) {
		EventSetRotation e = new EventSetRotation();
	...
	public void setPosition(double par1, double par3, double par5) {
		EventSetPosition e = new EventSetPosition();
		Client.INSTANCE.modManager.onEvent(e);
		if(!e.cancelled) {
			this.posX = par1;
			this.posY = par3;
	...
	public void moveEntity(double par1, double par3, double par5) {
		if (this.noClip) {
			...
			if (var20 || Client.INSTANCE.modManager.scaffold.isEnabled() && onGround) {
				double var21;
	...
	public boolean isWet() {
		IsPlayerInWaterEvent e = new IsPlayerInWaterEvent(this.inWater, this);
		Client.INSTANCE.modManager.onEvent(e);
		return e.isInWater() || this.worldObj.canLightningStrikeAt(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY), MathHelper.floor_double(this.posZ))
	...
	public boolean handleWaterMovement() {
		...
		if (this.worldObj.handleMaterialAcceleration(this.boundingBox.expand(0.0D, -0.4000000059604645D, 0.0D).contract(0.001D, 0.001D, 0.001D), Material.water, this)) {
			if (!e.isInWater() && !this.firstUpdate) {
	...
	public void setPositionAndRotation(double par1, double par3, double par5, float par7, float par8) {
		EventSetPositionAndRotation e = new EventSetPositionAndRotation();
		Client.INSTANCE.modManager.onEvent(e);
		if(!e.cancelled) {
	...
	public void setVelocity(double par1, double par3, double par5) {
		EventVelocity e = new EventVelocity();
		Client.INSTANCE.modManager.onEvent(e);
		if(!e.cancelled) {
			if(this.entityId == Minecraft.getMinecraft().thePlayer.entityId) {
				if(Client.INSTANCE.modManager.velocity.isEnabled() && Client.INSTANCE.modManager.velocity.novelocity.getValue()) return;
				this.motionX = Client.INSTANCE.modManager.velocity.isEnabled() ? par1*(Client.INSTANCE.modManager.velocity.horizontal.getValue()/100) : par1;
				this.motionY = Client.INSTANCE.modManager.velocity.isEnabled() ? par3*(Client.INSTANCE.modManager.velocity.vertical.getValue()/100) : par3;
				this.motionZ = Client.INSTANCE.modManager.velocity.isEnabled() ? par5*(Client.INSTANCE.modManager.velocity.horizontal.getValue()/100) : par5;
			} else {
				this.motionX = par1;
				this.motionY = par3;
				this.motionZ = par5;
