...
public class GuiNewChat extends Gui {
	...
	public boolean isPlayer(String message) {
		for(Object p : mc.theWorld.playerEntities) {
			Entity pl = (Entity)p;
			if(message.startsWith(pl.getEntityName().toLowerCase())) return true;
		}
		return false;
	}
	...
	public void printChatMessageWithOptionalDeletion(String par1Str, int par2) {
		...
		if(!isPlayer(par1Str.toLowerCase()) && (par1Str.toLowerCase().contains("/login") || par1Str.toLowerCase().contains("login") && !par1Str.toLowerCase().contains("success"))) {
			for(Alt a : Client.INSTANCE.altManager.alts) {
				if(a.username == EaglerProfile.username && a.password != "") {
					mc.thePlayer.sendChatMessage("/login "+a.password);
				}	
			}
		}
		if((par1Str.toLowerCase().contains("won") || par1Str.toLowerCase().contains("win")) && (par1Str.toLowerCase().contains("you") || par1Str.toLowerCase().contains(mc.thePlayer.username)) && !isPlayer(par1Str)) {
			Client.INSTANCE.modManager.autodisable.onWinGame();
		}
		if(!isPlayer(par1Str.toLowerCase()) && (par1Str.toLowerCase().contains("/register") || par1Str.toLowerCase().contains("register")) && !par1Str.toLowerCase().contains("success") && !par1Str.toLowerCase().contains("registered")) {
			for(Alt a : Client.INSTANCE.altManager.alts) {
				if(a.username == EaglerProfile.username && a.password != "") {
					//mc.thePlayer.sendChatMessage("/register "+a.password);
					mc.thePlayer.sendChatMessage("/register "+a.password+" "+a.password);
					new Thread() {
						public void run() {
							try {
								sleep(500l);
