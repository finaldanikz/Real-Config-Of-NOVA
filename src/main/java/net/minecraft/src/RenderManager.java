...
public class RenderManager {
	...
	public double[] dto2dprojection(double x, double y, double z) {
		Minecraft mc = Minecraft.getMinecraft();
		return new double[] {
				mc.gameSettings.fovSetting*x/z, mc.gameSettings.fovSetting*y/z
		};
	}
	...
	public void renderEntity(Entity par1Entity, float par2) {
		/*if(par1Entity instanceof EntityLiving) {
			...
			if(((EntityLiving)par1Entity) instanceof EntityOtherPlayerMP) {
				if(((EntityOtherPlayerMP)((EntityLiving)par1Entity)).isPlayerSleeping()) {
					var16 = -60;
				}else {
					int renderType = DefaultSkinRenderer.getPlayerRenderer((EntityOtherPlayerMP)((EntityLiving)par1Entity));
					if(renderType == 19) {
						var16 = -32;
			...
			EaglerAdapter.glTranslatef((float) par3, (float) par5 + ((EntityLiving)par1Entity).height / 2-0.2f, (float) par7);
			EaglerAdapter.glRotatef(-this.playerViewY, 0.0F, 1.0F, 0.0F);
			EaglerAdapter.glRotatef(this.playerViewX, 1.0F, 0.0F, 0.0F);
			...
			if(Client.INSTANCE.modManager.esp.isEnabled()) {
				EaglerAdapter.glColor3f((((EntityLiving)par1Entity).getMaxHealth()-((EntityLiving)par1Entity).health)/((EntityLiving)par1Entity).getMaxHealth(), 1-(((EntityLiving)par1Entity).getMaxHealth()-((EntityLiving)par1Entity).health)/((EntityLiving)par1Entity).getMaxHealth(), 0.0f);
				Gui.drawRect(-75 / 2-1, var16-60+10+(((EntityLiving)par1Entity).getMaxHealth()-((EntityLiving)par1Entity).health)/((EntityLiving)par1Entity).getMaxHealth()*80, (-75 / 2)-2, var16-60+90);
