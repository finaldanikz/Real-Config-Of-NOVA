...
public abstract class RenderLiving extends Render {
		...
		if (Minecraft.isGuiEnabled() && par1EntityLiving != this.renderManager.livingPlayer/* && par1EntityLiving.func_98034_c(Minecraft.getMinecraft().thePlayer)*/
			...
			if (var10 < (double) (var12 * var12)) {
				...
				if (par1EntityLiving.isSneaking()) {
					...
					EaglerAdapter.glDisable(EaglerAdapter.GL_TEXTURE_2D);
					EaglerAdapter.glDisable(EaglerAdapter.GL_ALPHA_TEST);
					if(!par1EntityLiving.func_98034_c(Minecraft.getMinecraft().thePlayer)) {
					...
					/*if(Client.INSTANCE.modManager.esp.isEnabled()) {
						EaglerAdapter.glColor3f((par1EntityLiving.getMaxHealth()-par1EntityLiving.health)/par1EntityLiving.getMaxHealth(), 1-(par1EntityLiving.getMaxHealth()-par1EntityLiving.health)/par1EntityLiving.getMaxHealth(), 0.0f);
						Gui.drawRect(-75 / 2-1, 10+(par1EntityLiving.getMaxHealth()-par1EntityLiving.health)/par1EntityLiving.getMaxHealth()*80, (-75 / 2)-2, 90);
						EaglerAdapter.glColor3f(Client.INSTANCE.modManager.esp.red.getValue()/255, Client.INSTANCE.modManager.esp.green.getValue()/255, Client.INSTANCE.modManager.esp.blue.getValue()/255);
						Gui.drawRect(-75 / 2, 0, (-75 / 2)+70, 1);
						Gui.drawRect(-75 / 2, 0, (-75 / 2)+1, 90);
						Gui.drawRect(-75 / 2+70, 0, (-75 / 2)+70+1, 90);
						Gui.drawRect(-75 / 2, 90-1, (-75 / 2)+70, 90);
					}*/
					EaglerAdapter.glAlphaFunc(EaglerAdapter.GL_GREATER, 0.1f);
					EaglerAdapter.glEnable(EaglerAdapter.GL_LIGHTING);
					EaglerAdapter.glDisable(EaglerAdapter.GL_BLEND);
	...
	protected void renderLivingLabel(EntityLiving par1EntityLiving, String par2Str, double par3, double par5, double par7, int par9) {
		...
		if (var10 <= (double) (par9 * par9)) {
			...
			if(!par1EntityLiving.func_98034_c(Minecraft.getMinecraft().thePlayer))
			var12.drawString(par2Str, -var12.getStringWidth(par2Str) / 2, (int) (var16), 553648127);
		...
		//System.out.println(par3 + "                " + par5 + "                     " +par7);
		/*//START
		EaglerAdapter.glPushMatrix();
		EaglerAdapter.glTranslatef((float) par3 + 0.0F, (float) par5 + par1EntityLiving.height / 2-0.2f, (float) par7);
		EaglerAdapter.glRotatef(-this.renderManager.playerViewY, 0.0F, 1.0F, 0.0F);
		EaglerAdapter.glRotatef(this.renderManager.playerViewX, 1.0F, 0.0F, 0.0F);
		EaglerAdapter.glScalef(-var14, -var14, var14);
		EaglerAdapter.glDisable(EaglerAdapter.GL_LIGHTING);
		...
		if(Client.INSTANCE.modManager.esp.isEnabled()) {
			EaglerAdapter.glColor3f((par1EntityLiving.getMaxHealth()-par1EntityLiving.health)/par1EntityLiving.getMaxHealth(), 1-(par1EntityLiving.getMaxHealth()-par1EntityLiving.health)/par1EntityLiving.getMaxHealth(), 0.0f);
			Gui.drawRect(-75 / 2-1, var16-60+10+(par1EntityLiving.getMaxHealth()-par1EntityLiving.health)/par1EntityLiving.getMaxHealth()*80, (-75 / 2)-2, var16-60+90);
			EaglerAdapter.glColor3f(Client.INSTANCE.modManager.esp.red.getValue()/255, Client.INSTANCE.modManager.esp.green.getValue()/255, Client.INSTANCE.modManager.esp.blue.getValue()/255);
			Gui.drawRect(-75 / 2, var16-60, (-75 / 2)+70, var16+1-60);
			Gui.drawRect(-75 / 2, var16-60, (-75 / 2)+1, var16+90-60);
			Gui.drawRect(-75 / 2+70, var16-60, (-75 / 2)+70+1, var16+90-60);
			Gui.drawRect(-75 / 2, var16+90-1-60, (-75 / 2)+70, var16+90-60);
		}
		EaglerAdapter.glEnable(EaglerAdapter.GL_LIGHTING);
		EaglerAdapter.glDepthMask(true);
		EaglerAdapter.glEnable(EaglerAdapter.GL_DEPTH_TEST);
	...
	protected void renderLivingLabel(EntityLiving par1EntityLiving, String par2Str, double par3, double par5, double par7, int par9, boolean draw) {
		float var13 = 1.6F;
		float var14 = 0.016666668F * var13;
		byte var16 = 0;
