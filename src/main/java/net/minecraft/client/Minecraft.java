...
public class Minecraft implements Runnable {
	...
	private void clickMouse(int par1) {
		if (par1 != 0 || this.leftClickCounter <= 0) {
			...
			if (par1 == 1) {
				if(Client.INSTANCE.modManager.fastplace.isEnabled()) {
            		this.rightClickDelayTimer = (int)Client.INSTANCE.modManager.fastplace.delay.getValue();
            	}
            	else 
            	{
				...
				if (EaglerAdapter.getEventKeyState()) {
					...
					if (EaglerAdapter.getEventKey() == 87) {
						...
						if (this.currentScreen != null) {
							...
							Client.INSTANCE.modManager.onKey(EaglerAdapter.getEventKey());
